...
class Alive_Prediction_Head(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_hidden_layers):
        ...
        for _ in range(num_hidden_layers):
            ...
            prev_dim = hidden_dim
        layers.append(nn.Linear(prev_dim, 10))  # Output layer for binary classification (win/loss)
        self.head = nn.Sequential(*layers)

    def forward(self, x):
        return self.head(x)  # shape: (batch_size, 10)

class Alive_Prediction_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = self.prediction_head(tick_features)  # shape: (batch, 10)
        return logits
...
def validate(model, dataloader, device, config):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Validating", leave=False):
            batch, nxt_kill, nxt_death, win_rate, target = batch_data
            target = target.to(device).float()  # shape: (batch, 10)
            batch = batch.to(device)
            
            # Forward pass - predict next ticks
            logits = model(batch)  # (batch, 10)
            ...
            # compute accuracy
            predictions = (logits > 0).float()  # (batch, 10)
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config/tfm_alive_fine-tuning.yaml', help='Path to the configuration file')
    ...
    prediction_head = Alive_Prediction_Head(
        input_dim=config['model']['embed_dim'] * 2,
        hidden_dim=config['model']['alive_hidden_dim'],
        num_hidden_layers=config['model']['alive_hidden_layers']
    ).to(device)
    ...
    model = Alive_Prediction_Model(base_model, prediction_head).to(device)
