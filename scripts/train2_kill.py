...
def validate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Validating", leave=False):
            ...
            nxt_kill = nxt_kill.to(device).long()  # shape: (batch,)
            ...
            # Compute loss
            loss = nn.functional.cross_entropy(logits, nxt_kill)
            total_loss += loss.item()
            num_batches += 1

            # compute accuracy
            predictions = logits.argmax(dim=-1)  # (batch,)
            correct_predictions += (predictions == nxt_kill).sum().item()
            total_predictions += nxt_kill.size(0)
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config/model2_kill.yaml', help='Path to the configuration file')
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        for batch_idx, batch_data in enumerate(progress_bar):
            ...
            train_accu = (logits.argmax(dim=-1) == nxt_kill).float().mean().item()
