...
def validate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Validating", leave=False):
            ...
            nxt_death = nxt_death.to(device).long()  # shape: (batch,)
            batch = batch.to(device)
            
            # Forward pass - predict next ticks
            logits = model(batch, None)  # (batch, 10)
            
            
            # Compute loss
            loss = nn.functional.cross_entropy(logits, nxt_death)
            total_loss += loss.item()
            num_batches += 1

            # compute accuracy
            predictions = logits.argmax(dim=-1)  # (batch,)
            correct_predictions += (predictions == nxt_death).sum().item()
            total_predictions += nxt_death.size(0)
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config/model2_death.yaml', help='Path to the configuration file')
    ...
    train_dataset = TickDataset2(args.config, split="train", shuffle_shards=True, skip_no_death=False, skip_no_alive=False)
    val_dataset = TickDataset2(args.config, split="val", shuffle_shards=False, skip_no_death=False, skip_no_alive=False)
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        for batch_idx, batch_data in enumerate(progress_bar):
            ...
            train_accu = (logits.argmax(dim=-1) == nxt_death).float().mean().item()
