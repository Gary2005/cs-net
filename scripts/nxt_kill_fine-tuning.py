...
from dataset.streaming_dataset_all_label import TickStreamingAllLabelsDataset
...
class Next_Kill_Prediction_Head(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_hidden_layers):
        ...
        for _ in range(num_hidden_layers):
            ...
            prev_dim = hidden_dim
        layers.append(nn.Linear(prev_dim, 22))  # Output layer for binary classification (win/loss)
        self.head = nn.Sequential(*layers)

    def forward(self, x):
        return self.head(x)  # shape: (batch_size, 22)

class Next_Kill_Prediction_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = self.prediction_head(tick_features)  # shape: (batch, 22)
        return logits
...
def validate(model, dataloader, device, config):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Validating", leave=False):
            batch, nxt_kill, nxt_death, win_rate, alive_target = batch_data
            target = torch.cat([nxt_kill, nxt_death], dim=0)  # shape: (batch*2,)
            target = target.to(device).long()  # shape: (batch*2,)
            batch = batch.to(device)  # (batch, ticks, seq_len)
            
            # Forward pass - predict next ticks
            logits = model(batch)  # (batch, 22)
            kill_logits = logits[:, :11]  # (batch, 11)
            death_logits = logits[:, 11:]  # (batch, 11)
            cat_logits = torch.cat([kill_logits, death_logits], dim=0)  # (batch*2, 11)
            
            # Compute loss
            loss = criterion(cat_logits, target)
            
            total_loss += loss.item()
            num_batches += 1

            # compute accuracy
            predictions = torch.argmax(cat_logits, dim=1)  # (2*batch, )
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config/tfm_nxt_kill_fine-tuning.yaml', help='Path to the configuration file')
    ...
    train_dataset = TickStreamingAllLabelsDataset(config_path=args.config, split="train", shuffle_shards=True)
    val_dataset = TickStreamingAllLabelsDataset(config_path=args.config, split="val", shuffle_shards=False)
    ...
    prediction_head = Next_Kill_Prediction_Head(
        input_dim=config['model']['embed_dim'] * 2,
        hidden_dim=config['model']['nxt_kill_hidden_dim'],
        num_hidden_layers=config['model']['nxt_kill_hidden_layers']
    ).to(device)
    ...
    model = Next_Kill_Prediction_Model(base_model, prediction_head).to(device)
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        for batch_idx, batch_data in enumerate(progress_bar):
            ...
            train_accu = (torch.argmax(cat_logits, dim=1) == target).float().mean().item()
