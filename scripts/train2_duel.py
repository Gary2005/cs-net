...
def validate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Validating", leave=False):
            ...
            player_i = player_i.to(device).unsqueeze(1)  # shape: (batch, 1)
            player_j = player_j.to(device).unsqueeze(1)  # shape: (batch, 1)
            player_ij = torch.cat((player_i, player_j), dim=1)  # shape: (batch, 2)
            label = label.to(device).float()  # shape: (batch, )
            batch = batch.to(device)
            
            # Forward pass - predict next ticks
            logits = model(batch, player_ij)  # (batch, 1)
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config/model2_duel.yaml', help='Path to the configuration file')
    ...
    train_dataset = TickDataset2(args.config, split="train", shuffle_shards=True, skip_no_death=True, skip_no_alive=False)
    val_dataset = TickDataset2(args.config, split="val", shuffle_shards=False, skip_no_death=True, skip_no_alive=False)
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        for batch_idx, batch_data in enumerate(progress_bar):
            ...
            train_accu = ((logits > 0).float() == label).float().mean().item()
