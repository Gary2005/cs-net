...
from dataset.streaming_dataset_all_label import TickStreamingAllLabelsDataset_Duel
...
def validate(model, dataloader, device, config):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Validating", leave=False):
            ...
            player_i, player_j, label = build_i_j(nxt_kill, nxt_death)

            player_i = player_i.to(device)  # shape: (batch, )
            player_j = player_j.to(device)  # shape: (batch, )
            target = label.to(device)  # shape: (batch, )
            batch = batch.to(device)  # (batch, ticks, seq_len)
            
            # Forward pass - predict next ticks
            logits = model(batch, player_i, player_j)  # (batch, )
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config/tfm_duel_fine-tuning.yaml', help='Path to the configuration file')
    ...
    train_dataset = TickStreamingAllLabelsDataset_Duel(config_path=args.config, split="train", shuffle_shards=True)
    val_dataset = TickStreamingAllLabelsDataset_Duel(config_path=args.config, split="val", shuffle_shards=False)
    ...
    prediction_head = Duel_Prediction_Head(
    ...
    model = Duel_Prediction_Model(base_model, prediction_head).to(device)
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        for batch_idx, batch_data in enumerate(progress_bar):
            ...
            train_accu = ((logits > 0).float() == target.float()).float().mean().item()
