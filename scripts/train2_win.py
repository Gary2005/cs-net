...
def validate(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Validating", leave=False):
            ...
            ct_win = ct_win.to(device).float()  # shape: (batch,)
            batch = batch.to(device)
            
            # Forward pass - predict next ticks
            logits = model(batch, None)  # (batch, 1)
            logits = logits.squeeze(-1)  # (batch,)
            
            
            # Compute loss
            loss = nn.functional.binary_cross_entropy_with_logits(logits, ct_win)
            ...
            correct_predictions += (predictions == ct_win).sum().item()
            total_predictions += ct_win.size(0)
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config/model2_win.yaml', help='Path to the configuration file')
    ...
    for epoch in range(start_epoch, config['training']['num_epochs']):
        ...
        for batch_idx, batch_data in enumerate(progress_bar):
            ...
            train_accu = ((logits > 0).float() == ct_win).float().mean().item()
