...
| 预测任务 | 说明 | 输出形式 |
|---------|------|---------|
| **胜率预测** | 当前时刻 team1（按当局攻防映射）赢下本局的概率 | 0~1 之间的概率值 |
| **存活预测** | 每个玩家在接下来 5 秒内仍然存活的概率 | 10 个玩家各一个概率值 |
| **下一次击杀预测** | 谁最可能拿到下一次击杀 | 10+1 个玩家的概率分布 |
| **下一次阵亡预测** | 谁最可能成为下一次阵亡者 | 10+1 个玩家的概率分布 |
...
> 修改集成，遵循上游 MIT 协议；我们主要适配了 CS-NET 的 Flask 路由与
> 模型预测叠加显示。
